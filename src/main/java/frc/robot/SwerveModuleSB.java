...
class SwerveModuleSB {
  ...
  private GenericEntry mAngleRateRps;
  ...
  public SwerveModuleSB(String name, SwerveModule module, ShuffleboardTab tab) {
    ...
    mAngleRateRps = tab.add(name + " Turn Rt Rps", 0).getEntry();
    mModule = module;
  }
  ...
  public void update() {
    ...
    mAngleRateRps.setDouble(mModule.getActualTurnRateRps());
    mSpeedDelta.setDouble(
        mModule.getState().speedMetersPerSecond - mModule.getDesiredState().speedMetersPerSecond);
  }
}
