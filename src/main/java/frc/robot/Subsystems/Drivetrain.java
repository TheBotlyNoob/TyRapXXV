...
public class Drivetrain extends SubsystemBase {
  ...
  protected final SimpleWidget m_fieldRelativeWidget =
      m_driveTab.add("drive field relative", fieldRelative);
  protected final GenericEntry m_driveCommandedRotationSpeed =
      m_driveTab.add("drive commanded rotation", 0).getEntry();

  /**
   * The order that you initialize these is important! Later uses of functions like
   * toSwerveModuleStates will return the same order that these are provided. See
  ...
  public Drivetrain(Pigeon2 gyro) {
    ...
    try {
      ...
      AutoBuilder.configure(
          ...
          this::resetOdo, // Method to reset odometry (will be called if your auto has a starting
          // pose)
          ...
          new PPHolonomicDriveController( // HolonomicPathFollowerConfig
              new PIDConstants(
                  DrivetrainConstants.drivePID[0], // Translation PID constants
                  DrivetrainConstants.drivePID[1],
                  DrivetrainConstants.drivePID[2]),
              new PIDConstants(
                  DrivetrainConstants.turnPID[0], // Rotation PID constants
  ...
  public void resetOdo() {
    ...
    if (DriverStation.getRawAllianceStation().equals(AllianceStationID.Blue1)
      ...
      m_odometry.resetPosition(
          ...
          getModulePositions(),
          new Pose2d(
              new Translation2d(getRoboPose2d().getX(), getRoboPose2d().getY()),
              new Rotation2d(Math.PI)));
    } else {
      m_odometry.resetPosition(
          new Rotation2d(),
          getModulePositions(),
          new Pose2d(
              new Translation2d(getRoboPose2d().getX(), getRoboPose2d().getY()), new Rotation2d()));
    }
  }
  ...
  /**
   * Module positions in the form of SwerveModulePositions (Module orientation and the distance the
   * wheel has travelled across the ground)
   *
    ...
    ChassisSpeeds chassisSpeeds =
        fieldRelative
            ? ChassisSpeeds.fromFieldRelativeSpeeds(
                xSpeed, ySpeed, rotSpeed, getPlayerStationRelativeYaw2d())
  ...
  public void driveChassisSpeeds(ChassisSpeeds chassisSpeeds) {
    m_driveCommandedRotationSpeed.setDouble(
  ...
  public void updateOdometry() {
    ...
    m_odometry.update(rotationYaw, getModulePositions());
    counter++;
    if (counter % 10 == 0 && enableVisionPoseInputs) {
      LimelightHelpers.SetRobotOrientation(
          ID.kFrontLimelightName, rotationYaw.getDegrees(), 0, 0, 0, 0, 0);
      LimelightHelpers.PoseEstimate mt2 =
          LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(ID.kFrontLimelightName);
      boolean doRejectUpdate = false;
      if (Math.abs(m_gyro.getAngularVelocityZWorld().getValueAsDouble())
          > 360) // if our angular velocity is too large, ignore vision updates
      {
        doRejectUpdate = true;
      }
      if (mt2 != null) {
  ...
  public void periodic() {
    ...
    SwerveModuleState[] swerveModuleStates =
        m_kinematics.toSwerveModuleStates(commandedChassisSpeeds);

    SwerveDriveKinematics.desaturateWheelSpeeds(
        swerveModuleStates, DrivetrainConstants.kMaxPossibleSpeed);
