...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        this.m_scoreLeft = buildScoreOffsetAutoCommand(true);

        this.m_scoreRight = buildScoreOffsetAutoCommand(false);
    ...
    public boolean shouldRemoveAlgae() {
        ...
        if (elevatorState == ElevatorLevel.LEVEL4) {
            if (Controller.kDriveController.getHID().getRightBumperButton()
                    || Controller.kDriveController.getHID().getLeftBumperButton())
                return true;
        }
        return false;
    }
