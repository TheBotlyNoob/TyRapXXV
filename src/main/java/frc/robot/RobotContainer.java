...
import frc.robot.Subsystems.algae.AlgaeRetrievalIOSpark;
...
public class RobotContainer {
  ...
  private StructArrayPublisher<SwerveModuleState> publisher =
      NetworkTableInstance.getDefault()
          ...
          .publish();
  protected GenericEntry m_driveP =
      m_competitionTab.add("Drive P Val", DrivetrainConstants.drivePID[0]).getEntry();
  protected GenericEntry m_driveFFStatic =
      m_competitionTab.add("Drive FF Static", DrivetrainConstants.driveFeedForward[0]).getEntry();
  protected GenericEntry m_driveFFVel =
      m_competitionTab.add("Drive FF Vel", DrivetrainConstants.driveFeedForward[1]).getEntry();
  protected GenericEntry m_driveAccel = m_competitionTab.add("Drive FF Accel", 0.0).getEntry();
  protected GenericEntry m_turnP =
      m_competitionTab.add("Turn P Val", DrivetrainConstants.turnPID[0]).getEntry();
  protected GenericEntry m_turnI =
      m_competitionTab.add("Turn I Val", DrivetrainConstants.turnPID[1]).getEntry();
  protected GenericEntry m_turnFFStatic =
      m_competitionTab.add("Turn FF Static", DrivetrainConstants.turnFeedForward[0]).getEntry();
  protected GenericEntry m_turnFFVel =
      m_competitionTab.add("Turn FF Vel", DrivetrainConstants.turnFeedForward[1]).getEntry();
  ...
  public RobotContainer() {
    ...
    this.m_algae =
        new AlgaeGrabberSubsystem(new AlgaePneumaticsIOReal(), new AlgaeRetrievalIOSpark());
  ...
  public void configureBindings() {
    ...
    Controller.kDriveController.back().toggleOnTrue(this.m_swerve.toggleFieldRelativeCommand());
    ...
    Controller.kDriveController
        .x()
        .and(m_climber::isCoralMode)
        .onTrue(m_coral.wristExtendCommand());
    Controller.kDriveController
        .b()
        .and(m_climber::isCoralMode)
        .onTrue(m_coral.wristRetractCommand());
    ...
    Controller.kDriveController
        ...
        .onTrue(
            new DriveDistance(
                ...
                () ->
                    (m_Limelight.getzDistanceMeters()
                            - Constants.Offsets.cameraOffsetFromFrontBumber)
                        + 0.02,
                0));
  ...
  private Command buildSelectRemoveAlgaeCommand() {
    return new SelectCommand<>(
        Map.ofEntries(
            Map.entry(
                CommandSelector.LOW,
                buildRemoveAlgaeCommand(ElevatorLevel.LEVEL3, ElevatorLevel.LEVEL5)),
            Map.entry(
                CommandSelector.HIGH,
                buildRemoveAlgaeCommand(ElevatorLevel.LEVEL4, ElevatorLevel.LEVEL6)),
  ...
  protected SequentialCommandGroup buildScoreOffsetCommand(boolean isLeft) {
    return new SequentialCommandGroup(
        ...
        new ConditionalCommand(
            ...
            new DriveFixedVelocity(
                    m_swerve,
                    180,
                    () -> m_elevator.getLevelFlag() == ElevatorLevel.LEVEL4 ? 1.5 : 3.0)
                .withTimeout(0.25),
            () -> (shouldRemoveAlgae())),
        m_elevator.runOnce(() -> m_elevator.setLevel(ElevatorLevel.GROUND)));
  }

  protected SequentialCommandGroup buildScoreBumperedUpCommand(
      boolean isLeft, double forwardTimeout) {
    return new SequentialCommandGroup(
        new ParallelCommandGroup(
  ...
  protected SequentialCommandGroup buildScoreBumperedUpAutoCommand(
      boolean isLeft, double forwardTimeout) {
    return new SequentialCommandGroup(
  ...
  public SequentialCommandGroup buildRemoveAlgaeCommand(
      ElevatorLevel startLevel, ElevatorLevel endLevel) {
    return new SequentialCommandGroup(
  ...
  private void configurePathPlanner() {
    ...
    autoChooser.addOption(
        "OnePieceAutoNoAlgae", "OnePieceAutoNoAlgae"); // Permanent choice //added line
  ...
  public SequentialCommandGroup buildTwoPieceAuto(
      String pathToReef,
      int tag1,
      String pathToCoralStn,
      String pathCoralToReef,
      int tag2,
      String pathToCoralStn2,
      double forwardDistM,
      boolean scoreLeft) {
    return new SequentialCommandGroup(
  ...
  public SequentialCommandGroup buildTwoPieceAutoBumpered(
      String pathToReef,
      int tag1,
      String pathToCoralStn,
      String pathCoralToReef,
  ...
  public void setPIDConstants() {
    ...
    for (SwerveModule m : m_swerve.getSwerveModules()) {
      ...
      m.getDriveFeedForward()
          .setKs(m_driveFFStatic.getDouble(Constants.DrivetrainConstants.driveFeedForward[0]));
      m.getDriveFeedForward()
          .setKv(m_driveFFVel.getDouble(Constants.DrivetrainConstants.driveFeedForward[1]));
      ...
      m.getTurnFeedForward()
          .setKs(m_turnFFStatic.getDouble(DrivetrainConstants.turnFeedForward[0]));
  ...
  public boolean shouldRemoveAlgae() {
    ...
    if (elevatorState == ElevatorLevel.LEVEL4) {
      if (Controller.kDriveController.getHID().getRightBumperButton()
          || Controller.kDriveController.getHID().getLeftBumperButton()) return true;
    }
    return false;
  }
